...
function displayError(value: string | string[] | undefined) {
  ...
  if (!text) return null;
  if (text === "simulado-invalido") return "O simulado selecionado é inválido.";
    ...
    <main className={styles.page} id="main-content">
      <header className={styles.intro}>
        <div>
          ...
          <span>Cada tentativa preserva as questões e a ordem do início ao resultado.</span>
        </div>
      ...
      {!access.hasFullAccess && (
        <section className={styles.allowance}>
          <div>
            ...
            <strong>{catalog.freeAttempt ? "Simulado já utilizado" : "1 simulado disponível"}</strong>
          </div>
          <p>
            {catalog.freeAttempt
              ? "Você pode retomar ou consultar o resultado da tentativa já iniciada."
              : "Ao iniciar, esta tentativa ficará vinculada à sua conta e poderá ser retomada depois."}
          </p>
          {catalog.freeAttempt && (
            <Link href={`/simulados/${catalog.freeAttempt.id}${catalog.freeAttempt.status === "SUBMITTED" ? "/resultado" : ""}`}>
              {catalog.freeAttempt.status === "SUBMITTED" ? "Ver resultado →" : "Continuar tentativa →"}
            </Link>
          )}
        </section>
      )}
      ...
      <section className={styles.grid} aria-label="Catálogo de simulados">
        {catalog.simulations.map((simulation, index) => {
          ...
          return (
            <article className={!simulation.available ? styles.locked : ""} key={simulation.id}>
              <div className={styles.cardIndex}>{String(index + 1).padStart(2, "0")}</div>
              <div className={styles.cardTop}>
                <Badge variant={simulation.access === "FREE" ? "success" : "premium"}>
                  {simulation.access === "FREE" ? "Gratuito" : "Completo"}
                </Badge>
                <span>{simulation.durationMinutes ? `${simulation.durationMinutes} min` : "Sem cronômetro"}</span>
              </div>
              <h2>{simulation.title}</h2>
              <p>{simulation.description ?? "Simulado organizado para acompanhar seu desempenho."}</p>

              <div className={styles.cardActions}>
                {latest && (
                  <Link href={`/simulados/${latest.id}${latest.status === "SUBMITTED" ? "/resultado" : ""}`}>
                    {latest.status === "SUBMITTED" ? "Ver último resultado" : "Continuar simulado"}
                  </Link>
                )}

                {simulation.available && (!latest || access.hasFullAccess) ? (
                  <form action={startSimulationAction}>
                    ...
                    <button type="submit">{latest ? "Nova tentativa" : "Iniciar simulado"} →</button>
                  </form>
                ) : !simulation.available ? (
                  <Link className={styles.upgrade} href="/planos">Liberar acesso →</Link>
                ) : null}
              </div>
            </article>
          );
      ...
      {!catalog.simulations.length && (
        <section className={styles.empty}>
          ...
          <p>Execute o seed inicial ou publique um simulado pela área administrativa.</p>
        </section>
      )}
    </main>
  );
